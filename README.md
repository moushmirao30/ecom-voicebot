...
flowchart LR
    ...
    Agent --> LLM[Gemini 2.5 Flash<br/>⇄ NVIDIA Llama-3.1-8B<br/>FallbackAdapter]
    Agent --> TTS[Cartesia Sonic<br/>TTS]
...
**LLM failover and routing.** Gemini 2.5 Flash is primary for quality; a
`FallbackAdapter` transparently retries NVIDIA's `meta/llama-3.1-8b-instruct`
on any primary error, so a quota blip degrades a turn instead of killing it.
Optional per-step routing sends tool-decision turns to the fast/cheap model
and user-facing replies to the higher-quality one.
...
The test suite (31 passing: 19 network-free unit + 12 live-LLM evals) combines
deterministic tool/value assertions with LLM judges for hallucination,
off-topic refusal, and multi-turn grounding. Two bugs it caught:
...
| LLM | Gemini 2.5 Flash → NVIDIA Llama-3.1-8B | Quality primary, resilient fallback (~481 ms TTFT) |
| TTS | Cartesia Sonic | Fast, natural inflection |
