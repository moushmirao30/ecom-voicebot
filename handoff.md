...
>
> **Remaining, in order:** (6) **redeploy the fixed stack** — `lk agent deploy` from WSL (L8–L11 code) **and add `TTS_PROVIDER=deepgram` to the cloud agent's secrets** (the deployed agent has its own env; without it the audio glitch persists in production), then merge the frontend branch to `main` so Vercel picks up the scroll/badge fixes; (7) **record the 60–90s demo video** — everything verified locally 2026-07-13; follow the shot list + L8–L11 regression pass in `demo/DEMO_SCRIPT.md`; (8) the other ~9 **full-flow runs** against the deployed stack (voice-driven); (9) **open the backend PR** `feat/ui-suggested-direction` → `main` (CI is green; frontend already merges to `main` per step 6).

---

## 🔥 Live-Deploy Bug-Fix Log (2026-07-04 → 07-08)

Bugs found by driving the **deployed** stack (cloud agent + Vercel frontend), plus **local `localhost` test runs on 2026-07-08** (screen recordings `Test_run 1.mp4`, `The audio glitch.mp4`) that surfaced L8–L11. Every fix ships with a regression test where testable. **All eleven (L1–L11) verified live** — L8–L11 confirmed in a localhost re-run on **2026-07-13** (scroll, badge, policy answers, result count, and clean Aura audio all good). ⚠️ The **deployed** stack hasn't picked up L8–L11 yet: redeploy the cloud agent (`lk agent deploy` from WSL) and merge the frontend to `main` for Vercel. Fast suite now **50 passed**. The cloud agent auto-scales to zero when idle, so each fix = `lk agent deploy` from WSL; the frontend auto-deploys from `main` via the GitHub↔Vercel integration.
