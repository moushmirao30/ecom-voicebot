...
> ⚠️ **Voice note for the demo:** Cartesia Sonic is out of free credits, so **Max currently speaks with Deepgram Aura** (still natural, different voice than Sonic). The failover makes the demo *work* regardless. To restore the Sonic voice, add credits at play.cartesia.ai (or wait for a plan reset); no code change needed — Cartesia is still the primary and resumes automatically once it has credits.
...
**Test suite: 54 passed** (42 network-free unit + 12 live-LLM evals). Run: `pytest` (all) or `pytest -m "not llm"` (fast, no keys). *Unit total grew with the live-deploy fixes above: `test_order_id.py` (13, L2), `test_category_filter.py` (5, L5), `test_stock_price_match.py` (4, L7), plus routing/cart.*
