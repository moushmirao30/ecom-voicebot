    ...
    def __init__(self, route_llms: bool = True) -> None:
        super().__init__(
            instructions=(
                ...
                "- POLICY QUESTIONS ARE ALWAYS IN SCOPE. Any question about returns, refunds, exchanges, shipping, delivery, payment options, cash on delivery, EMI, warranty, cancellations, damaged products, or rewards MUST be answered by calling `policy_lookup` on THIS turn — even if the topic came up before. NEVER refuse a policy question as off-topic, and NEVER claim you already answered it instead of answering.\n"
                "- When answering a policy question, DO NOT read the policy content verbatim — summarize the 2-3 facts that answer the question (e.g. for returns: the 7-day window, the unused-with-tags condition, free pickup) and offer to share more details if they want. Long word-for-word recitals take too long to speak.\n"
